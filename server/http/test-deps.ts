...
export function createTestDeps(overrides: Partial<Record<keyof Deps, unknown>> = {}): Deps {
  ...
  const base = {
    ...
    agentTokens: {
      ...
      findSigningKey: vi.fn().mockResolvedValue(null),
      createSigningKey: vi.fn(),
      consumeGrantUse: vi.fn().mockResolvedValue(true),
      createApproval: vi.fn(),
      findApproval: vi.fn().mockResolvedValue(null),
      approveApproval: vi.fn().mockResolvedValue(null),
    ...
    externalAccounts: {
      createAccountWithCredential: vi.fn(),
      listByOwnerUser: vi.fn().mockResolvedValue([]),
      listByOwnerAgents: vi.fn().mockResolvedValue([]),
      findAccount: vi.fn().mockResolvedValue(null),
      findCredential: vi.fn().mockResolvedValue(null),
      updateCredential: vi.fn().mockResolvedValue(null),
      createGrant: vi.fn(),
      findGrant: vi.fn().mockResolvedValue(null),
      findActiveGrant: vi.fn().mockResolvedValue(null),
      revokeGrant: vi.fn().mockResolvedValue(false),
      createOAuthIntent: vi.fn(),
      consumeOAuthIntent: vi.fn().mockResolvedValue(null),
    },
