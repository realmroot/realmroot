...
export function createTestDeps(overrides: Partial<Record<keyof Deps, unknown>> = {}): Deps {
  ...
  const base = {
    ...
    externalResources: {
      ...
      claimConnectionRefresh: vi.fn().mockResolvedValue(true),
      completeConnectionRefresh: vi.fn().mockResolvedValue({ id: 'connection-refreshed' }),
      releaseConnectionRefresh: vi.fn().mockResolvedValue(true),
