...
const providerConnectionMigrationName = '20260808144344_abnormal_newton_destine.sql'
const providerConnectionAuthorityMigrationName = '20260808160850_lowly_norman_osborn.sql'
...
describe('tenant ownership migration', () => {
  it('rebuilds legacy ownership into the final schema and quarantines ambiguity', () => {
    ...
    try {
      for (const name of migrationNames().filter(
        (name) =>
          ![
            ...
            providerConnectionAuthorityMigrationName,
          ].includes(name),
      )) {
      ...
      database.exec(
        readFileSync(new URL(`../../migrations/${providerConnectionAuthorityMigrationName}`, import.meta.url), 'utf8'),
      )
