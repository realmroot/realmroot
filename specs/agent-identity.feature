Feature: Agent identity and external API authorization
  ...
  Rule: External API resources use target-issued authorization
    ...
    Scenario: An administrator associates an external API resource with an OIDC connector
      ...
      When an administrator creates the external API resource and associates that connector
