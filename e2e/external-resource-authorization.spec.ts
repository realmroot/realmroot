...
test.describe('external API resource authorization', () => {
  ...
  test(`[spec: agent-identity/native-api-resource-token]
    ...
    try {
      ...
      const keyPair = await generateKeyPair('ES256', { extractable: true })
      const publicJwk = await exportJWK(keyPair.publicKey)
      const tokenUrl = `${baseURL}/api/agent/access-grants/${approved.grantId}/tokens`
      const tokenProof = await dpopProof('POST', tokenUrl, keyPair.privateKey, publicJwk)
      const lease = await plugin.agentRequest<{
        accessToken: string
        tokenType: 'DPoP'
        permissions: string[]
      ...
      const targetUrl = `${flareauthResource}/projects`
      const resourceProof = await dpopProof('GET', targetUrl, keyPair.privateKey, publicJwk, lease.accessToken)
      const direct = await fetch(targetUrl, {
        headers: { authorization: `DPoP ${lease.accessToken}`, dpop: resourceProof },
      })
