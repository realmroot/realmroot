...
test('[spec: account-center/hosted-application-sessions] preserves the application through sign-in and removes one login', async ({
  ...
  const first = await login(page, clientId)
  const second = await login(page, clientId)
  ...
  await expect(page.getByRole('heading', { name: 'Application login sessions' })).toBeVisible()
  ...
  await expect(page.getByText('2 active login sessions', { exact: true })).toBeVisible()
  await expect(page.getByText(/not a verified physical device/)).toBeVisible()
  ...
  const remove = page.getByRole('button', { name: 'Remove session', exact: true }).first()
  await remove.focus()
  ...
  await expect(page.getByText('2 active login sessions', { exact: true })).toBeVisible()
  await remove.click()
  await dialog.getByRole('button', { name: 'Remove session', exact: true }).click()
  await expect(dialog).not.toBeVisible()
  await expect(page.getByText('1 active login session', { exact: true })).toBeVisible()
  await expect(page.getByText(/Login session removed\./)).toBeVisible()
  const refreshed = await Promise.all(
    [first, second].map((refreshToken) =>
  ...
  expect(refreshed.map((result) => result.status()).sort()).toEqual([200, 400])
})

async function login(page: Page, clientId: string) {
