import { zhAccountTranslations } from './i18n-zh-account'
import { zhFlatTranslationsExtraA } from './i18n-zh-extra-a'
import { zhFlatTranslationsExtraB } from './i18n-zh-extra-b'

export const zhFlatTranslations: Record<string, string> = {
  Account: '账户',
  'Account Center': '账户中心',
  'Account details': '账户详情',
  'Account operations': '账户操作',
  'Account recovery': '账户恢复',
  'Account settings': '账户设置',
  Actions: '操作',
  Active: '启用',
  'Active sessions': '活跃会话',
  Admin: '管理员',
  'Admin Console': '管理控制台',
  'All organizations': '所有组织',
  'API resource': 'API 资源',
  'API resources': 'API 资源',
  Applications: '应用',
  Approve: '批准',
  'Approve access': '批准访问',
  Audience: '受众',
  'Auth issuer': '认证签发方',
  'Authenticator app': '认证器应用',
  'Authenticator code': '认证器验证码',
  'Authorized applications': '已授权应用',
  'Authorized apps': '已授权应用',
  Available: '可用',
  Back: '返回',
  'Back to API resources': '返回 API 资源',
  'Back to applications': '返回应用',
  'Back to organizations': '返回组织',
  'Back to roles': '返回角色',
  'Back to sign in': '返回登录',
  'Back to users': '返回用户',
  'Backup codes': '备份码',
  Ban: '封禁',
  'Ban user': '封禁用户',
  Banned: '已封禁',
  Branding: '品牌',
  CAPTCHA: '验证码',
  Cancel: '取消',
  Callback: '回调',
  'Callback URL': '回调 URL',
  Change: '更改',
  'Change email': '更改邮箱',
  'Change password': '更改密码',
  Changes: '变更',
  'Check your inbox': '检查你的收件箱',
  'Checking callback state': '正在检查回调状态',
  'Choose file': '选择文件',
  Claims: '声明',
  'Client ID': '客户端 ID',
  'Client secret': '客户端密钥',
  Close: '关闭',
  Code: '验证码',
  Configuration: '配置',
  Connect: '连接',
  'Connected accounts and apps': '已连接账户和应用',
  'Connections and apps': '连接和应用',
  Connectors: '连接器',
  'Console setup': '控制台设置',
  Continue: '继续',
  'Continue to sign in': '继续登录',
  'Continue with': '继续使用',
  'Continue with Email': '使用邮箱继续',
  'Continue with OneTap': '使用 OneTap 继续',
  'Continue with Passkey': '使用通行密钥继续',
  'Continue with Phone': '使用手机号继续',
  'Continue with Web3 wallet': '使用 Web3 钱包继续',
  Copy: '复制',
  'Copy client config': '复制客户端配置',
  'Copy client secret': '复制客户端密钥',
  'Copy details': '复制详情',
  Create: '创建',
  'Create API resource': '创建 API 资源',
  'Create OIDC client': '创建 OIDC 客户端',
  'Create account': '创建账户',
  'Create application': '创建应用',
  'Create endpoint': '创建端点',
  'Create first admin': '创建首个管理员',
  'Create organization': '创建组织',
  'Create the Role first, then assign Scopes from its detail page.': '先创建角色，然后在角色详情页中分配 Scope。',
  'Create role': '创建角色',
  'Create user': '创建用户',
  Created: '已创建',
  'Current password': '当前密码',
  'Custom JWT': '自定义 JWT',
  Dashboard: '仪表盘',
  Database: '数据库',
  'Developer Center': '开发者中心',
  Delete: '删除',
  'Delete application': '删除应用',
  'Delete connector': '删除连接器',
  'Delete passkey': '删除通行密钥',
  'Delete resource': '删除资源',
  'Delete role': '删除角色',
  'Delete user': '删除用户',
  Deleting: '正在删除',
  Deny: '拒绝',
  Description: '描述',
  Disable: '禁用',
  Verify: '验证',
  Disabled: '已禁用',
  'Disabled reason': '禁用原因',
  Discard: '放弃',
  Display: '显示',
  'Display name': '显示名称',
  Done: '完成',
  Edit: '编辑',
  'Edit avatar': '编辑头像',
  'Edit email': '编辑邮箱',
  'Set up': '设置',
  'Set up authenticator app': '设置认证器应用',
  'Sign out others': '退出其他设备',
  'Edit profile': '编辑资料',
  'Edit username': '编辑用户名',
  Email: '邮箱',
  'Email or username': '邮箱或用户名',
  'Email verification': '邮箱验证',
  'Email verification code': '邮箱验证码',
  Enable: '启用',
  Enabled: '已启用',
  Endpoint: '端点',
  'Endpoint URL': '端点 URL',
  Endpoints: '端点',
  'Enroll authenticator app': '注册认证器应用',
  Error: '错误',
  Event: '事件',
  Events: '事件',
  Failed: '失败',
  Filter: '筛选',
  General: '常规',
  'General security': '常规安全',
  'General settings': '常规设置',
  Global: '全局',
  'Grant types': '授权类型',
  Hide: '隐藏',
  'Hide password': '隐藏密码',
  'Hosted authentication preview': '托管认证预览',
  'Hosted messages': '托管文案',
  'Hosted sign-in': '托管登录',
  Identifier: '标识符',
  'Identity summary': '身份摘要',
  'Initial password': '初始密码',
  Issuer: '签发方',
  Key: '键',
  Lifecycle: '生命周期',
  'Linked accounts': '关联账户',
  Links: '链接',
  Loading: '正在加载',
  'Loading Console dashboard': '正在加载控制台仪表盘',
  'Loading consent request': '正在加载授权请求',
  'Loading sign-in options': '正在加载登录选项',
  Logo: 'Logo',
  'Logo URL': 'Logo URL',
  'MFA and passkeys': 'MFA 和通行密钥',
  'MFA enforcement': 'MFA 强制策略',
  'MFA policy': 'MFA 策略',
  'MFA state': 'MFA 状态',
  Management: '管理',
  'Management API': '管理 API',
  'API Documentation': 'API 文档',
  Mobile: '移动端',
  'Multi-factor authentication': '多因素认证',
  'Multi-factor auth': '多因素认证',
  Name: '名称',
  'New application': '新应用',
  'New organization': '新组织',
  'New password': '新密码',
  'New resource': '新资源',
  'New role': '新角色',
  'New user': '新用户',
  Next: '下一步',
  'No account yet?': '还没有账户？',
  'No active sessions.': '没有活跃会话。',
  'No active sessions': '暂无活跃会话',
  'New sign-ins will appear here.': '新的登录会话会显示在这里。',
  'No other active sessions.': '没有其他活跃会话。',
  'No applications yet': '暂无应用',
  'No authorized applications yet.': '暂无已授权应用。',
  'No authorized applications': '暂无已授权应用',
  'Applications you approve will appear here.': '你授权的应用会显示在这里。',
  'No connected resource accounts': '暂无已连接的资源账户',
  'Resource accounts connected for Agent access will appear here.': '为 Agent 访问连接的资源账户会显示在这里。',
  'No Agent identities': '暂无 Agent 身份',
  'Agent identities you control will appear here.': '你管理的 Agent 身份会显示在这里。',
  'No pending requests': '暂无待处理请求',
  'New resource access requests will appear here.': '新的资源访问请求会显示在这里。',
  'No Agent activity': '暂无 Agent 活动',
  'Agent activity will appear here when events are available.': '产生事件后，Agent 活动会显示在这里。',
  'No linked accounts.': '暂无关联账户。',
  'No organizations yet': '暂无组织',
  'Create an organization or accept an invitation to get started.': '创建组织或接受邀请即可开始使用。',
  'No passkeys have been added yet.': '尚未添加通行密钥。',
  'Add a passkey to sign in without a password.': '添加通行密钥，即可免密码登录。',
  'Use a passkey for fast, passwordless sign-in.': '使用通行密钥快速免密码登录。',
  'This browser is your only active session.': '当前浏览器是你唯一的活跃会话。',
  'Protect your account with an authenticator app.': '使用认证器应用保护你的账户。',
  'No roles yet': '暂无角色',
  'No sign-in connectors are available.': '暂无可用登录连接器。',
  'No additional sign-in methods': '暂无其他登录方式',
  'Your organization has not enabled social or OAuth sign-in for this account.':
    '你的组织尚未为此账户启用社交或 OAuth 登录。',
  'No sign-in methods are enabled. Contact the Realm operator.': '未启用登录方式。请联系 Realm 管理员。',
  'Not set up': '尚未设置',
  'No users yet': '暂无用户',
  Off: '关闭',
  'One-time code': '一次性验证码',
  Optional: '可选',
  Organization: '组织',
  Organizations: '组织',
  Overview: '概览',
  Password: '密码',
  Passwordless: '免密码',
  Passkey: '通行密钥',
  Passkeys: '通行密钥',
  Pending: '待处理',
  Permissions: '权限',
  Phone: '手机号',
  Platform: '平台',
  Previous: '上一步',
  Privacy: '隐私',
  'Privacy URL': '隐私 URL',
  Profile: '资料',
  'Profile settings': '资料设置',
  Provider: '提供商',
  Reason: '原因',
  Recommended: '推荐',
  Redirect: '重定向',
  'Redirect URIs': '重定向 URI',
  Reject: '拒绝',
  Remove: '移除',
  Request: '请求',
  Required: '必填',
  'Realm-wide administration': 'Realm 全局管理',
  Resend: '重新发送',
  'Resend code': '重新发送验证码',
  'Resend code in {{seconds}}s': '{{seconds}} 秒后重新发送验证码',
  Resource: '资源',
  Retry: '重试',
  Revoke: '撤销',
  'Revoke all': '全部撤销',
  'Revoke all sessions': '撤销所有会话',
  'Revoke other sessions': '撤销其他会话',
  'Revoke session': '撤销会话',
  Role: '角色',
  Roles: '角色',
  Save: '保存',
  'Save changes': '保存更改',
  'Save OIDC claims': '保存 OIDC 声明',
  'Save profile': '保存资料',
  Saving: '正在保存',
  'Saving…': '正在保存…',
  Scope: '范围',
  Scopes: '范围',
  Search: '搜索',
  Secret: '密钥',
  Security: '安全',
  Sessions: '会话',
  Settings: '设置',
  Showing: '显示',
  'Show password': '显示密码',
  'Sign in': '登录',
  'Sign in to Realmroot': '登录 Realmroot',
  'Sign in to continue.': '登录以继续。',
  'Sign out': '退出登录',
  'Sign up': '注册',
  'Sign-in': '登录',
  'Sign-in & account': '登录和账户',
  'Sign-in security': '登录安全',
  'Sign-up': '注册',
  'Sign-up and sign-in': '注册和登录',
  Slug: 'Slug',
  'Switch organization': '切换组织',
  Status: '状态',
  Subject: '主体',
  Support: '支持',
  'Support email': '支持邮箱',
  System: '系统',
  Tenant: '租户',
  'Terms URL': '条款 URL',
  'Third-party apps': '第三方应用',
  Token: '令牌',
  Type: '类型',
  Unlink: '解绑',
  Unverified: '未验证',
  Updated: '已更新',
  'Upload image': '上传图片',
  Use: '使用',
  User: '用户',
  Users: '用户',
  Username: '用户名',
  'Verification code': '验证码',
  Verified: '已验证',
  'Verify code': '验证验证码',
  'Verify your email.': '验证你的邮箱。',
  Version: '版本',
  Webhooks: 'Webhook',
  'Use one of the enabled methods to access this application.': '使用已启用的方式访问此应用。',
  'Use your account to continue securely.': '使用你的账户安全继续。',
  'Start with your identity.': '从你的身份开始。',
  'Recover your password.': '恢复你的密码。',
  'Request a one-time code and set a new password for your account.': '请求一次性验证码并为你的账户设置新密码。',
  'Request failed.': '请求失败。',
  'Request failed': '请求失败',
  'Invalid form input.': '表单输入无效。',
  'Account update failed.': '账户更新失败。',
  'Unable to load account center.': '无法加载账户中心。',
  'Unable to load consent request.': '无法加载授权请求。',
  'Unable to approve consent.': '无法批准授权。',
  'Unable to switch accounts.': '无法切换账户。',
  'Onboarding failed.': '初始化失败。',
  'First-admin onboarding is already locked.': '首个管理员初始化已锁定。',
  'First admin created. Sign in to finish Console setup.': '首个管理员已创建。请登录以完成控制台设置。',
  'Completing sign-in': '正在完成登录',
  'Sign-in could not continue.': '登录无法继续。',
  'Consent is required before redirecting.': '重定向前需要授权。',
  'Sign-in complete.': '登录完成。',
  'Create an account for the requested application.': '为请求的应用创建账户。',
  'Recover access for the requested application.': '为请求的应用恢复访问权限。',
  'Verify your email for the requested application.': '为请求的应用验证邮箱。',
  'Continue to the requested application.': '继续访问请求的应用。',
  'Create a hosted account to continue.': '创建托管账户以继续。',
  'Recover your hosted account before continuing.': '继续前请恢复你的托管账户。',
  'Confirm your email address before continuing.': '继续前请确认你的邮箱地址。',
  'Sign in with your hosted account to continue.': '使用你的托管账户登录以继续。',
  'Create an account for {{destination}}.': '为 {{destination}} 创建账户。',
  'Recover access for {{destination}}.': '恢复对 {{destination}} 的访问。',
  'Verify your email for {{destination}}.': '验证用于 {{destination}} 的邮箱。',
  'Continue to {{destination}}.': '继续访问 {{destination}}。',
  'Create a hosted account to continue to {{destination}}.': '创建托管账户以继续访问 {{destination}}。',
  'Recover your hosted account before continuing to {{destination}}.':
    '继续访问 {{destination}} 前请恢复你的托管账户。',
  'Confirm your email address before continuing to {{destination}}.': '继续访问 {{destination}} 前请确认你的邮箱地址。',
  'Sign in with your hosted account to continue to {{destination}}.':
    '使用你的托管账户登录以继续访问 {{destination}}。',
  'Confirm your identity with this provider.': '使用此提供商确认你的身份。',
  'Share basic profile details such as name and avatar.': '共享姓名和头像等基础资料。',
  'Share your email address and verification state.': '共享你的邮箱地址和验证状态。',
  'Allow refresh tokens for continued access.': '允许刷新令牌以保持访问。',
  'Allow this application to request this scope.': '允许此应用请求该范围。',
  'OAuth client application': 'OAuth 客户端应用',
  'Current account': '当前账户',
  'Provider metadata must be a JSON object.': '提供商元数据必须是 JSON 对象。',
  'Custom data JSON must be an object.': '自定义数据 JSON 必须是对象。',
  'Federated credentials': '联合凭据',
  'Add federated credential': '添加联合凭据',
  'Add credential': '添加凭据',
  'JWKS URL': 'JWKS URL',
  'Public keys': '公钥',
  'Select an API resource': '选择一个 API 资源',
  'No federated credentials yet.': '暂无联合凭据。',
  'public key(s)': '个公钥',
  'Let an external workload exchange its own OIDC token for an access token, with no client secret to manage.':
    '让外部工作负载用自身的 OIDC 令牌换取访问令牌，无需管理客户端密钥。',
  'Trusted issuer and subject pairs that may request tokens for this application.':
    '可为此应用请求令牌的受信任签发方与主体组合。',
  'Logical issuer identity, an opaque string. Not dereferenced as a URL.':
    '逻辑签发方标识，为不透明字符串，不会作为 URL 解析。',
  'Exact subject, or a prefix ending in * to match a range.': '精确主体，或以 * 结尾的前缀以匹配一个范围。',
  'The API resource this credential may request tokens for.': '此凭据可为其请求令牌的 API 资源。',
  'Public keys are fetched from this URL.': '将从此 URL 获取公钥。',
  'Or paste a JWK or JWK Set as JSON. Provide a JWKS URL or public keys, not both.':
    '或以 JSON 形式粘贴 JWK 或 JWK Set。提供 JWKS URL 或公钥，但不要同时提供两者。',
  'Provide either a JWKS URL or inline public keys, not both.': '请提供 JWKS URL 或内联公钥，但不要同时提供两者。',
  'A federated credential requires either a JWKS URL or one or more public keys.':
    '联合凭据需要 JWKS URL 或一个及以上公钥。',
  'Public keys must be a valid JWK or JWK Set in JSON format.': '公钥必须是有效的 JWK 或 JWK Set（JSON 格式）。',
}

Object.assign(zhFlatTranslations, zhFlatTranslationsExtraA, zhFlatTranslationsExtraB, zhAccountTranslations)
